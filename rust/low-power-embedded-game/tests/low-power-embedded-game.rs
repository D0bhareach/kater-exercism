mod divmod {
    //! tests of divmod
    //!
    //! note that we're only testing positive quantities; no need to get into
    //! the mod/rem distinction
...
mod evens {
    use low_power_embedded_game::{even_string_parts, evens, odds, transform};

    #[test]
    fn simple_i32() {
        let out: Vec<i32> = evens(0..)
    ...
    #[test]
    fn reverse_i32() {
        let out: Vec<i32> = transform((0..=10).rev()).collect();
    ...
    #[ignore]
    fn offset_i32() {
        let out: Vec<i32> = odds(1..).take(5).collect();
    ...
    fn strs() {
        ...
        let out: Vec<_> = even_string_parts(input).collect();
